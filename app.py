...
def atms_app():
    """Приложение подключается к хранилищу для получения подготовленных данных
    о городе. Принимает от пользователя время работы устройства для расчета баллов
    по этому критерию. Вычисляет итоговую оценку каждой локации города и
    отображает на карте."""
